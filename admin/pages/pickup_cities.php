...
$sql=$con->query("SELECT * FROM pickup_cities ORDER BY id") or die("Error2 : ". mysqli_error($con));
$count = mysqli_num_rows($sql); 
?>
       <?php echo number_format($count); ?> Pickup Cities
      </h2>
      <p><a href="#"><i class="fa fa-dashboard"></i> Home</a> &nbsp;&nbsp; > &nbsp;&nbsp; <a class="active">Pickup Cities</a></p>
    </div>
     <div class="col-md-4" style="text-align: left; margin-top: 10px;">  <a href="?p=new_pickup_city"> <button class="btn btn-primary" style="font-size: 16px; font-weight: 600;">Add Pickup City</button></a>  </div>
     <div class="col-md-4" style="text-align: left; margin-top: 10px;">  <a href="?p=pickup_area"> <button class="btn btn-primary" style="font-size: 16px; font-weight: 600;">Add Pickup Area</button></a>  </div>
...
<?php
  ...
  $rows = fetchAllPickupState($conn); 

  if(isset($_GET['q']))
  {
    $q = $_GET['q'];
    $rows = searchPickupCity($conn, $q);
